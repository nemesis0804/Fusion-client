...
function RestrictionsTab() {
    ...
    const fetchRestrictionsList = async () => {
      try {
        ...
        showApiError({
          ...
          fallback: "Failed to fetch restrictions list",
          authorizationFallback:
            "Only placement officer users can manage placement restrictions.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async () => {
    ...
    if (editingRestriction) {
      try {
        const response = await placementApi.updateRestriction(
          editingRestriction.id,
          restrictionData,
        );

        setRestrictions(
          restrictions.map((r) =>
            r.id === editingRestriction.id ? response.data : r,
          ),
        );
        notifications.show({
          title: "Success",
        ...
        showApiError({
          error,
          fallback: "Failed to update restriction.",
          authorizationFallback:
            "Only placement officer users can manage placement restrictions.",
        });
        return;
      ...
      try {
        const response = await placementApi.createRestriction(restrictionData);
        ...
        showApiError({
          error,
          fallback: "Failed to add restriction.",
          authorizationFallback:
            "Only placement officer users can manage placement restrictions.",
        });
      }
  ...
  const handleDelete = async (id) => {
    try {
      await placementApi.deleteRestriction(id);
      ...
      showApiError({
        error,
        fallback: "Failed to delete restriction.",
        authorizationFallback:
          "Only placement officer users can manage placement restrictions.",
      });
    }
